...
-- TRIGGER para registrar alterações em Vendas no histórico
--CREATE TRIGGER trg_AfterUpdate_Venda
--ON Venda
--AFTER UPDATE
--AS
--BEGIN
--    INSERT INTO HistoricoVenda (CodV, CodFC, CodL, ValorLivro, TeveDesconto, ValorFinal, DataVenda, CodFP, UsuarioModificacao)
--    SELECT i.CodV, i.CodFC, i.CodL, i.ValorLivro, i.TeveDesconto, i.ValorFinal, i.DataVenda, i.CodFP, SUSER_NAME()
--    FROM inserted i;
--END;
--GO

---- TRIGGER para registrar ações importantes no sistema
--CREATE TRIGGER trg_Historico_Acao
--ON Livro
--AFTER INSERT, UPDATE, DELETE
--AS
--BEGIN
--    DECLARE @Acao VARCHAR(10), @Descricao VARCHAR(255);
...
--    SET @Descricao = 'Ação ' + @Acao + ' na tabela Livro.';

--    INSERT INTO HistoricoAcao (TabelaAfetada, TipoAcao, DescricaoAcao, UsuarioAcao)
--    VALUES ('Livro', @Acao, @Descricao, SUSER_NAME());
--END;
--GO
